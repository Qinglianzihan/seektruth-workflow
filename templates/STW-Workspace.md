...
求是工作流在决策环节实行"民主集中制"——用两个独立角色确保方案质量：

### 规划师（阶段 2/3）

在**阶段 2（抓住主要矛盾）** 和**阶段 3（集中优势兵力）**，建议调用独立的「规划师」子代理对方案进行独立规划：

> 调用方式（Claude Code）：创建一个新会话，将 `规划师.md` 作为系统提示，提供调研报告和初步方案。

规划师将独立检查：
- 主要矛盾是否从调研中准确提炼
- ATTACK_ZONE 是否最小且必要
- 变更计划是否每个文件都有明确理由
- 是否存在范围蔓延（"顺路改一下"的无关变更）
...
> 规划在前，审查在后——这是"从群众中来，到群众中去"在 AI 编程中的制度化。
